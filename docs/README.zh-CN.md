# Bye Bye Steven

[English](../README.md) | **[简体中文](README.zh-CN.md)** | [繁體中文](README.zh-TW.md) | [日本語](README.ja.md) | [한국어](README.ko.md)

> 让 Steven 永远消失。

一个 Windows 工具，用于移除 [Mewgenics](https://store.steampowered.com/app/686060/Mewgenics/) 中 **Steven** 的反 SL 惩罚。

Steven 是一个能检测到你重新加载存档的 NPC，然后会出来惩罚你。这个工具一键重置惩罚。

<p align="center">
  <img src="screenshot.png" alt="Bye Bye Steven" width="400">
</p>

## 功能

- **一键移除惩罚** — 立即重置 Steven 标记
- **自动备份** — 修改前自动备份存档
- **重启并清除** — 关闭游戏、移除惩罚、重新启动一步到位
- **自动检测路径** — 自动检测存档文件和游戏程序路径
- **5 种语言** — English, 简体中文, 繁體中文, 日本語, 한국어

## 原理

Mewgenics 的存档文件是一个 SQLite 数据库。当游戏检测到存档重载时，会将 `properties` 表中的 `savescumlocation` 字段设为 `1`，从而触发 Steven 出现。本工具只是将该值重置为 `0`。

**测试版本：** Mewgenics `v1.0.20695`（截至 2026 年 2 月最新版）

> [!WARNING]
> 未来的游戏更新可能会更改存档文件结构，这可能导致本工具失效或产生意外行为。游戏更新后请检查本工具是否有新版本。

## 下载

前往 [Releases](https://github.com/TaliesinYang/mewgenics-bye-bye-steven/releases) 页面下载最新版本。

单个便携 `.exe`，无需安装。

## 使用方法

1. 运行 `ByeByeSteven.exe`
2. 存档路径会自动检测
3. 点击 **移除惩罚** 重置 Steven 标记
4. 或使用 **重启并清除** 一键关闭游戏、清除惩罚并重启

> [!IMPORTANT]
> **使用前请自行备份存档文件。** 虽然工具会自动创建备份，但强烈建议手动备份。存档文件位于：
> ```
> %APPDATA%\Glaiel Games\Mewgenics\mewgenics.sav
> ```

## 从源码构建

需要 [.NET 9 SDK](https://dotnet.microsoft.com/download/dotnet/9.0)。

```bash
dotnet publish src/MewgenicsSaveGuardian.csproj -c Release -o publish
```

## 免责声明

本工具按「现状」提供，不附带任何形式的担保。作者**不对**因使用本工具造成的任何数据丢失、存档损坏或其他损失负责。**使用风险自负。**

## 许可证

[CC BY-NC 4.0](../LICENSE) — 可自由使用和修改，禁止商用。
